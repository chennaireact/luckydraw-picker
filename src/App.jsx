  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const p of pts) {
        ...
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.s * p.z, 0, Math.PI * 2);
      ...
      <div className="brand-row">
        ...
        <div>
          <h1 className="brand-title">
            Lucky Draw <span className="highlight">Picker</span>
          </h1>
      ...
      >
        <div className="input-columns">
          <div className="input-col-form">
            <label className="input-label" htmlFor="contest-name">
              Event Name
            </label>
            <input
              id="contest-name"
...
function SpinStep({ names, contestName, spinDuration, onBack, onWinner }) {
  ...
  const handleSpin = useCallback(() => {
    ...
    const totalRounds = Math.max(
      40,
      Math.ceil((spinDuration * 1000) / (ITEM_H * names.length)),
    );
      ...
      <div className="spin-header">
        ...
        <div>
          ...
          <p className="spin-subtitle">
            {names.length} participant{names.length !== 1 ? "s" : ""} &middot;{" "}
            {spinDuration}s spin
          </p>
        </div>
      </div>
            ...
            <div className="reel-scroll-area">
              {visibleItems.map((item) => (
                ...
                >
                  <span
                    className={
                      item.isCenter ? "reel-item-center" : "reel-item-dim"
                    }
                  >
                    {item.text}
                  </span>
...
function WinnerOverlay({ winner, contestName, onDrawAgain, onNewList }) {
  ...
  return (
    ...
    >
      ...
      >
        <button className="btn-primary" onClick={onDrawAgain}>
          Draw Again
        </button>
        <button
        ...
        <button className="btn-ghost" onClick={onNewList}>
          New List
        </button>
      </motion.div>
    </motion.div>
